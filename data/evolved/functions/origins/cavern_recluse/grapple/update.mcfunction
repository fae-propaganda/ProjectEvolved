...
# Tag an armor stand
execute as @e[type=marker,tag=ev.grapplehook] if score @s ev.id = @p ev.id run tag @s add ev.this.grapplehook
...
execute facing entity @e[type=marker,tag=ev.grapplehook,tag=ev.this.grapplehook,sort=nearest,limit=1] feet run function evolved:origins/cavern_recluse/grapple/render_web
